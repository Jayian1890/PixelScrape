...
MSECrypto::MSECrypto()
    : dh_(nullptr, DH_free), encrypt_ctx_(nullptr, EVP_CIPHER_CTX_free),
...
bool MSECrypto::compute_shared_secret(
  ...
  BIGNUM *peer_pub_bn =
      BN_bin2bn(peer_public_key.data(), peer_public_key.size(), nullptr);
  if (!peer_pub_bn) {
    return false;
  }
  ...
  int secret_len =
      DH_compute_key(shared_secret_.data(), peer_pub_bn, dh_.get());
  BN_free(peer_pub_bn);
...
void MSECrypto::init_rc4_encryption(const std::vector<uint8_t> &key) {
  ...
  // Initialize RC4
  if (EVP_EncryptInit_ex(encrypt_ctx_.get(), EVP_rc4(), nullptr, key.data(),
                         nullptr) != 1) {
...
void MSECrypto::init_rc4_decryption(const std::vector<uint8_t> &key) {
  ...
  // Initialize RC4
  if (EVP_DecryptInit_ex(decrypt_ctx_.get(), EVP_rc4(), nullptr, key.data(),
                         nullptr) != 1) {
...
void MSECrypto::encrypt(std::vector<uint8_t> &data) {
  ...
  std::vector<uint8_t> out(data.size() +
                           EVP_CIPHER_CTX_block_size(encrypt_ctx_.get()));

  if (EVP_EncryptUpdate(encrypt_ctx_.get(), out.data(), &out_len, data.data(),
                        data.size()) != 1) {
    return;
  }
...
void MSECrypto::decrypt(std::vector<uint8_t> &data) {
  ...
  int out_len;
  std::vector<uint8_t> out(data.size() +
                           EVP_CIPHER_CTX_block_size(decrypt_ctx_.get()));

  if (EVP_DecryptUpdate(decrypt_ctx_.get(), out.data(), &out_len, data.data(),
                        data.size()) != 1) {
    return;
  }
...
std::vector<uint8_t>
MSECrypto::compute_crypto_select(const std::array<uint8_t, 20> &info_hash,
...
MSECrypto::derive_rc4_keys(const std::array<uint8_t, 20> &info_hash,
                           const std::vector<uint8_t> &shared_secret) const {

  // KeyA = HASH('keyA', S, SKEY)
  std::vector<uint8_t> dataA;
  dataA.insert(dataA.end(), {'k', 'e', 'y', 'A'});
  dataA.insert(dataA.end(), shared_secret.begin(), shared_secret.end());
  dataA.insert(dataA.end(), info_hash.begin(), info_hash.end());
  auto keyA_hash = sha1(dataA);
  std::vector<uint8_t> keyA(keyA_hash.begin(), keyA_hash.end());

  // KeyB = HASH('keyB', S, SKEY)
  std::vector<uint8_t> dataB;
  dataB.insert(dataB.end(), {'k', 'e', 'y', 'B'});
  dataB.insert(dataB.end(), shared_secret.begin(), shared_secret.end());
  dataB.insert(dataB.end(), info_hash.begin(), info_hash.end());
  auto keyB_hash = sha1(dataB);
  std::vector<uint8_t> keyB(keyB_hash.begin(), keyB_hash.end());
