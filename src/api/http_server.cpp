...
void HttpServer::run() {
  while (running_) {
    ...
    int client_socket =
        accept(server_socket_, reinterpret_cast<sockaddr *>(&client_addr),
               &client_len);
    if (client_socket < 0) {
      if (running_) {
        // Log error
...
HttpRequest HttpServer::parse_request(int client_socket) {
  ...
  while (std::getline(header_stream, line, '\n')) {
    ...
    if (colon_pos != std::string::npos) {
      ...
      header_name.erase(
          std::find_if(header_name.rbegin(), header_name.rend(),
                       [](unsigned char c) { return !std::isspace(c); })
              .base(),
          header_name.end());
      ...
      header_value.erase(
          ...
          std::find_if(header_value.begin(), header_value.end(),
                       [](unsigned char c) { return !std::isspace(c); }));
      header_value.erase(
          std::find_if(header_value.rbegin(), header_value.rend(),
                       [](unsigned char c) { return !std::isspace(c); })
              .base(),
          header_value.end());
...
void HttpServer::send_response(int client_socket,
