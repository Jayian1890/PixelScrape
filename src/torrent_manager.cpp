...
void TorrentManager::tracker_worker(const std::string& torrent_id) {
    while (true) {
        ...
        // Capture raw pointer to avoid dangling reference if map entry is removed
        Torrent* torrent_ptr = it->second.get();

        // Check stopping condition safely
        {
            std::lock_guard<std::mutex> t_lock(torrent_ptr->mutex);
            if (torrent_ptr->stopping) break;
        }

        if (torrent_ptr->paused) {
            lock.unlock();
            std::unique_lock<std::mutex> t_lock(torrent_ptr->mutex);
            if (torrent_ptr->cv.wait_for(t_lock, std::chrono::seconds(5), [&](){ return torrent_ptr->stopping; })) break;
            continue;
        }
        ...
        try {
            ...
            auto peers = torrent_ptr->tracker->get_peers(
                torrent_ptr->peer_id,
                6881, // Default port
                torrent_ptr->uploaded_bytes,
                torrent_ptr->downloaded_bytes,
                torrent_ptr->metadata.total_length - torrent_ptr->downloaded_bytes,
                "" // No event for now
            );

            {
                ...
                for (const auto& peer_info : peers) {
                    bool known = false;
                    for (const auto& existing : torrent_ptr->discovered_peers) {
                    ...
                    if (!known) {
                        torrent_ptr->discovered_peers.push_back(peer_info);
                    }
                }
            }
    ...
    while (true) {
        ...
        // 1. Manage Peers
        {
            ...
            for (const auto& peer_info : discovered_copy) {
                ...
                for (const auto& existing : peers_copy) {
                    if (existing->get_peer_info().ip == peer_info.ip && existing->get_peer_info().port == peer_info.port) {
                ...
                // Also check new connections we are about to make
                for (const auto& existing : new_connections) {
                    if (existing->get_peer_info().ip == peer_info.ip && existing->get_peer_info().port == peer_info.port) {
                        already_connected = true;
                        break;
                    }
