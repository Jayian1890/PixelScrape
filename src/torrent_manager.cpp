...
void TorrentManager::tracker_worker(const std::string& torrent_id) {
    ...
    while (true) {
        ...
        if (first_run) {
            pixellib::core::logging::Logger::info("Connecting to {} tracker(s) for torrent {}", 
                tracker_urls.size(), torrent_id.substr(0, 8));
        }
        ...
        for (const auto& tracker_url : tracker_urls) {
            ...
            try {
                auto peers = torrent->tracker->get_peers(
                    ...
                    first_run ? "started" : "", // Send "started" event on first announce
                ...
                pixellib::core::logging::Logger::warning("Tracker {} error for {}: {}", 
                    tracker_url.substr(0, 50), torrent_id.substr(0, 8), e.what());
            }
        }

        if (total_new_peers > 0 || first_run) {
            std::lock_guard<std::mutex> t_lock(torrent->mutex);
            pixellib::core::logging::Logger::info("Discovered {} new peer(s) for torrent {} (total: {})", 
                total_new_peers, torrent_id.substr(0, 8), torrent->discovered_peers.size());
        }
