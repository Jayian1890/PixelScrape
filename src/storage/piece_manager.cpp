...
PieceManager::PieceManager(const TorrentMetadata &metadata,
    ...
    : metadata_(metadata), download_dir_(download_dir),
      file_priorities_(file_priorities),
      have_pieces_(metadata.piece_hashes.size(), false),
      active_downloads_(metadata.piece_hashes.size()), disk_running_(true),
  ...
  try {
    ...
    for (const auto &file : metadata.files) {
      ...
      try {
        ...
        if (!out_file) {
          pixellib::core::logging::Logger::error("Failed to create file: {}",
                                                 file_path);
        ...
        if (file.length > 0) {
          ...
          if (!out_file) {
            pixellib::core::logging::Logger::error(
                "Failed to write sparse file: {}", file_path);
            throw std::runtime_error("Failed to write sparse file: " +
                                     file_path);
          }
        }
        out_file.close();
      } catch (const std::exception &e) {
        pixellib::core::logging::Logger::error("Exception creating file {}: {}",
                                               file_path, e.what());
        throw;
      }
    }
    ...
    pixellib::core::logging::Logger::error(
        "Failed to initialize PieceManager: {}", e.what());
    throw;
  }
}
...
size_t PieceManager::select_rarest_piece(
    const std::vector<std::vector<bool>> &peer_bitfields,
  ...
  for (const auto &bitfield : peer_bitfields) {
    if (bitfield.size() != metadata_.piece_hashes.size())
      continue;
  ...
  for (size_t i = 0; i < piece_file_mappings_.size(); ++i) {
    for (const auto &mapping : piece_file_mappings_[i]) {
      piece_priorities[i] =
          std::max(piece_priorities[i], file_priorities_[mapping.file_index]);
    }
  }
  ...
  for (size_t i = 0; i < availability.size(); ++i) {
    if (!have_pieces_[i] && availability[i] > 0) {
      // Check filter mask
      if (filter_mask) {
        if (i >= filter_mask->size() || !(*filter_mask)[i]) {
          continue;
        }
      }

      // Check if piece is already fully requested
      bool fully_requested = false;
      if (i < active_downloads_.size()) {
        const auto &d = active_downloads_[i];
        if (!d.blocks_requested.empty()) {
          fully_requested =
              std::all_of(d.blocks_requested.begin(), d.blocks_requested.end(),
                          [](bool b) { return b; });
        }
      }
      ...
      if (availability[i] < min_availability) {
        better = true;
      } else if (availability[i] == min_availability &&
                 piece_priorities[i] > max_priority) {
        better = true;
      }
      if (better) {
...
bool PieceManager::request_block(size_t piece_index, size_t block_index,
                                 size_t block_size) {
  std::lock_guard<std::mutex> lock(pieces_mutex_);

  if (piece_index >= active_downloads_.size())
    return false;
  ...
  if (download.blocks_received.empty()) {
    ...
    size_t piece_size = (piece_index == metadata_.piece_hashes.size() - 1)
                            ? (metadata_.total_length % metadata_.piece_length)
                            : metadata_.piece_length;
    if (piece_size == 0)
      piece_size = metadata_.piece_length;
  ...
  if (block_index >= download.blocks_requested.size())
    return false;
  if (download.blocks_requested[block_index])
...
bool PieceManager::receive_block(size_t piece_index, size_t begin,
  ...
  if (block_index >= download.blocks_received.size())
    return false;
  if (download.blocks_received[block_index])
    return false;
...
bool PieceManager::is_piece_complete(size_t piece_index) const {
  ...
  return std::all_of(download.blocks_received.begin(),
                     download.blocks_received.end(),
                     [](bool received) { return received; });
}

bool PieceManager::is_piece_fully_requested(size_t piece_index) const {
  std::lock_guard<std::mutex> lock(pieces_mutex_);

  if (piece_index >= active_downloads_.size())
    return false;

  // If we already have the piece, it is considered fully requested
  if (have_pieces_[piece_index])
    return true;

  const auto &download = active_downloads_[piece_index];
  if (download.blocks_requested.empty())
    return false;

  return std::all_of(download.blocks_requested.begin(),
                     download.blocks_requested.end(),
...
bool PieceManager::verify_piece(size_t piece_index) {
  ...
  // Check if piece is complete without calling is_piece_complete to avoid
  // deadlock
  if (piece_index >= active_downloads_.size())
    return false;
  const auto &download = active_downloads_[piece_index];
  if (download.blocks_received.empty())
    return false;
  if (!std::all_of(download.blocks_received.begin(),
                   download.blocks_received.end(),
                   [](bool received) { return received; }))
    return false;
  ...
  if (hash == metadata_.piece_hashes[piece_index]) {
    ...
    // Add write request to disk queue
    {
      ...
      disk_queue_.push(
          DiskRequest{DiskRequest::WRITE, piece_index, 0, piece_data, {}});
    }
    disk_cv_.notify_one();
  ...
  try {
    ...
    for (const auto &mapping : piece_file_mappings_[piece_index]) {
      std::string file_path =
          download_dir_ + "/" + metadata_.files[mapping.file_index].path;
      ...
      if (!file) {
        pixellib::core::logging::Logger::error("Failed to seek in file: {}",
                                               file_path);
        return {};
      }

      file.read(reinterpret_cast<char *>(piece_data.data() + offset),
                mapping.length);
      if (file.gcount() != static_cast<std::streamsize>(mapping.length)) {
        pixellib::core::logging::Logger::error(
            "Failed to read complete block from file: {}", file_path);
        return {};
      }
    ...
    pixellib::core::logging::Logger::error("Exception reading piece {}: {}",
                                           piece_index, e.what());
    return {};
  }
}

bool PieceManager::write_piece(size_t piece_index,
                               const std::vector<uint8_t> &data) {
  try {
    ...
    for (const auto &mapping : piece_file_mappings_[piece_index]) {
      ...
      if (!file) {
        pixellib::core::logging::Logger::error(
            "Failed to seek in file for writing: {}", file_path);
        return false;
      }
      ...
      file.write(reinterpret_cast<const char *>(data.data() + offset),
                 mapping.length);
      if (!file) {
        pixellib::core::logging::Logger::error("Failed to write to file: {}",
                                               file_path);
        return false;
      }
      offset += mapping.length;
    ...
    pixellib::core::logging::Logger::error("Exception writing piece {}: {}",
                                           piece_index, e.what());
    return false;
  }
}
...
double PieceManager::get_completion_percentage() const {
  if (metadata_.total_length == 0)
    return 0.0;
  return static_cast<double>(get_total_downloaded_bytes()) /
         metadata_.total_length * 100.0;
}
...
size_t PieceManager::get_total_downloaded_bytes() const {
  ...
  for (size_t i = 0; i < have_pieces_.size(); ++i) {
    if (have_pieces_[i]) {
      size_t piece_size =
          (i == have_pieces_.size() - 1)
              ? (metadata_.total_length -
                 (have_pieces_.size() - 1) * metadata_.piece_length)
              : metadata_.piece_length;
      total += piece_size;
    }
  }
...
void PieceManager::disk_worker() {
  while (disk_running_) {
    ...
    disk_cv_.wait(lock,
                  [this]() { return !disk_queue_.empty() || !disk_running_; });
    ...
    try {
      ...
      if (request.type == DiskRequest::READ) {
        ...
        if (!write_piece(request.piece_index, request.data)) {
          pixellib::core::logging::Logger::error(
              "Failed to write piece {} to disk", request.piece_index);
        }
      }
    } catch (const std::exception &e) {
      pixellib::core::logging::Logger::error(
          "Exception in disk_worker for piece {}: {}", request.piece_index,
          e.what());
    }
  }
}
...
void PieceManager::calculate_file_mappings() {
  ...
  for (size_t file_idx = 0; file_idx < metadata_.files.size(); ++file_idx) {
    ...
    while (file_remaining > 0 &&
           current_piece < metadata_.piece_hashes.size()) {
      ...
      piece_file_mappings_[current_piece].push_back(
          {file_idx, file.length - file_remaining, bytes_in_this_piece});
...
std::pair<size_t, size_t> PieceManager::get_file_range(size_t piece_index,
                                                       size_t offset,
                                                       size_t length) const {
