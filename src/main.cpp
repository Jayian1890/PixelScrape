...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    pixelscrape::TransmissionRpcHandler transmission_handler(&torrent_manager, torrent_manager.get_download_dir());
    ...
    http_server.add_route("POST", "/transmission/rpc", [&transmission_handler](const pixelscrape::HttpRequest& req) { return transmission_handler.handle_request(req); });
    ...
    pixellib::core::logging::Logger::info("PixelScrape started - HTTP server on port 8080, WebSocket on port "
                                          "8081");
