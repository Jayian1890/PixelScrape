...
public:
  ...
  // Piece management
  size_t
  select_rarest_piece(const std::vector<std::vector<bool>> &peer_bitfields,
                      const std::vector<bool> *filter_mask = nullptr);
  bool request_block(size_t piece_index, size_t block_index, size_t block_size);
  bool receive_block(size_t piece_index, size_t begin,
                     const std::vector<uint8_t> &data);
  bool is_piece_complete(size_t piece_index) const;
  bool is_piece_fully_requested(size_t piece_index) const;
...
private:
  ...
  std::pair<size_t, size_t> get_file_range(size_t piece_index, size_t offset,
                                           size_t length) const;
