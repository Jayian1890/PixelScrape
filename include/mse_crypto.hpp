...
public:
  ...
  std::vector<uint8_t>
  compute_crypto_provide(const std::array<uint8_t, 20> &info_hash,
                         const std::vector<uint8_t> &shared_secret) const;

  std::vector<uint8_t>
  compute_crypto_select(const std::array<uint8_t, 20> &info_hash,
                        const std::vector<uint8_t> &shared_secret,
                        uint32_t crypto_select) const;

  // Derive RC4 keys (A->B and B->A)
  // Returns pair {initiator_key, receiver_key}
  std::pair<std::vector<uint8_t>, std::vector<uint8_t>>
  derive_rc4_keys(const std::array<uint8_t, 20> &info_hash,
                  const std::vector<uint8_t> &shared_secret) const;

  const std::vector<uint8_t> &get_shared_secret() const {
